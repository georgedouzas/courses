...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  // Previous and current nodes set to head
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  // the function.
  LinkedList<T> merged;
  Node *leftCurrentNode = left.head_;
  Node *rightCurrentNode = right.head_;
  T data;
  bool flipLastNode;
  
  // Merge left empty list
  if (left.size_ == 0 and right.size_ > 0) {
    merged = right;
  }
  // Merge right empty list
  else if (right.size_ == 0 and left.size_ > 0) {
    merged = left;
  }
  // Merge same size lists
  else if (right.size_ == left.size_) {
    
    // Iterate through nodes
    while (leftCurrentNode) {
      
      // Left list node is larger than right list node
      if (leftCurrentNode->data >= rightCurrentNode->data) {
        flipLastNode = merged.size_ > 0 and rightCurrentNode->data < merged.tail_->data;
        if (flipLastNode) {
          data = merged.tail_->data;
          merged.popBack();
        }
        merged.pushBack(rightCurrentNode->data);
        if (flipLastNode)
          merged.pushBack(data);
        merged.pushBack(leftCurrentNode->data);
      }

      // Right list node is larger than left list node
      else {
        flipLastNode = merged.size_ > 0 and leftCurrentNode->data < merged.tail_->data;
        if (flipLastNode) {
          data = merged.tail_->data;
          merged.popBack();  
        }
    ...
    // Iterate through right list nodes
    while (rightCurrentNode) {
    ...
    // Iterate through remaining left list nodes
    while (leftCurrentNode) {
      flipLastNode = leftCurrentNode->data < merged.tail_->data;
      if (flipLastNode) {
        data = merged.tail_->data;
        merged.popBack();  
      }
    ...
    // Iterate through left list nodes
    while (leftCurrentNode) {
    ...
    // Iterate through remaining right list nodes
    while (rightCurrentNode) {
      flipLastNode = rightCurrentNode->data < merged.tail_->data;
      if (flipLastNode) {
        data = merged.tail_->data;
        merged.popBack();  
      }
